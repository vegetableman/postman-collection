...
_.extend(Response.prototype, /** @lends Response.prototype */ {
    ...
    size: function() {
        var size = 0,
            contentLength = Header.getHeaderValue(this.headers.members, 'Content-Length');

        // if 'Content-Length' header present
        if (contentLength) {
            size = _.parseInt(contentLength);
        }
        // else size of body added
        else {
            size = this.stream ? this.stream.byteLength : (this.body && this.body.length || 0);
        }

        // size of header added
        size += Header.getSize(this.headers.members);
        return size;
    }
});
